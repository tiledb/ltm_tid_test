...
def find_calibration(csv_file):
    with open(csv_file, newline="") as f:
        ...
        for row_num, row in enumerate(reader, start=1):
            ...
            if msg.startswith("MB calibration factors:"):
                factors = ast.literal_eval(msg.split(":", 1)[1].strip())

                if len(factors) != len(CHANNELS):
                    raise RuntimeError("Wrong number of calibration values.")
...
def find_measurements(csv_file):
    ...
    with open(csv_file, newline="") as f:
        ...
        for line_num, row in enumerate(reader, start=1):
            ...
            try:
                values = [float(x) for x in row[5:5 + len(CHANNELS)]]
            except Exception:
                continue
    ...
    if len(rows) != 3:
        raise RuntimeError("Could not find three valid measurement rows.")
...
def compute(csv_file):
    ...
    for i in range(len(CHANNELS)):
        averages.append(
            average([row[1][i] for row in rows])
        )
    ...
    if not replaced:
        raise RuntimeError("Calibration line not found.")

    #
    # Recompute every measurement row
    #
    ...
    for i, line in enumerate(lines):
        ...
        for ch in range(len(CHANNELS)):
            ...
            corrected = raw / new_cal[ch]

            values[ch] = round(corrected, 2)
        #
        # Replace the values in the row
        #
...
def main():
    ...
    group.add_argument(
        ...
        "--compute",
        action="store_true",
        help="Compute and update calibration factors"
    )

    args = parser.parse_args()

    process(args.path, args.show)
